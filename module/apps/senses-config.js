...
export default class ActorSensesConfig extends DocumentSheet {
  ...
  /** @inheritdoc */
  get title() {
    return `${game.i18n.localize("PERGASHA.SensesConfig")}: ${this.document.name}`;
  }
  ...
  getData(options) {
    ...
    const data = {
      ...
      units: senses.units, movementUnits: CONFIG.PERGASHA.movementUnits
    };
    for (let [name, label] of Object.entries(CONFIG.PERGASHA.senses)) {
      const v = senses[name];
      data.senses[name] = {
