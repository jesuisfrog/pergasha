...
export const migrateWorld = async function () {
  ...
  game.settings.set("pergashaFoundryvtt", "systemMigrationVersion", game.system.data.version);
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.PERGASHA.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if (actorData.flags.pergashaFoundryvtt) {
    actorData.flags.pergashaFoundryvtt = filterObject(actorData.flags.pergashaFoundryvtt, allowedFlags);
  }
...
function _migrateActorSenses(actor, updateData) {
  ...
  for (let s of original.split(",")) {
    ...
    if (type in CONFIG.PERGASHA.senses) {
...
function _migrateActorType(actor, updateData) {
  ...
  if (match) {
    ...
    const typeMatch = Object.entries(CONFIG.PERGASHA.creatureTypes).find(([k, v]) => {
      return (typeLc === k) ||
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("PERGASHA.CreatureSwarm"));
    if (match.groups.size || isNamedSwarm) {
      const sizeLc = match.groups.size ? match.groups.size.trim().toLowerCase() : "tiny";
      const sizeMatch = Object.entries(CONFIG.PERGASHA.actorSizes).find(([k, v]) => {
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.PERGASHA.attunementTypes.NONE;
...
export async function purgeFlags(pack) {
  const cleanFlags = (flags) => {
    const flags5e = flags.pergashaFoundryvtt || null;
    return flags5e ? { pergashaFoundryvtt: flags5e } : {};
  };
  await pack.configure({ locked: false });
